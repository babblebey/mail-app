...
- **Navigate back after marking as unread**: When a user marks a message as unread, they are redirected back to the mail list for the current folder — mirroring the Delete/Report Spam behaviour. Since the user is signalling they want to revisit the message later, staying on a now-unread thread would be inconsistent. Marking as read (the less common case from within the thread view) keeps the user on the thread.
