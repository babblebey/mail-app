...
Chrome Performance panel trace was captured on 2026-04-22 for thread image-heavy render from:

- `Trace-20260422T015820.json`
...
| mail-thread.image-heavy-render | `2.5 ms` dominant `mail-thread.message-body` mount commit (`0.8 ms`, `0.8 ms`, `2.5 ms`) | `1` commit per observed message-body mount | `48.1 ms` median (`40.5 ms`, `44.3 ms`, `47.4 ms`, `48.8 ms`, `181.3 ms`, `217.9 ms`) | Yes (`Trace-20260422T015820.json`: renderer main `9` long tasks `>= 50 ms`, max `390.1 ms`) | Median meets target, but high outliers (`181.3 ms`, `217.9 ms`) and browser long tasks confirm occasional heavy-path stalls. |
...
| `Trace-20260422T015820.json` | Thread image-heavy render capture set | `21268` | `10` | `9` | `390.1 ms` | `17` |
| `Trace-20260422T014504-composer.json` | Composer typing + recipient-edit capture set | `35544` | `86` | `85` | `1676.4 ms` | `25` |

All captured traces show renderer-main long tasks above the `>= 50 ms` threshold, confirming main-thread blocking during these interaction paths.
...
- The image-heavy-render capture meets the `<= 120 ms` median target (`48.1 ms`) but has major outliers (`181.3 ms`, `217.9 ms`) and browser-confirmed renderer long tasks (max `390.1 ms`), so it is not yet stable.
- Observed behavior in thread image-heavy flow: opening attachment preview and then closing the dialog causes inline images in the thread body to disappear and re-render, indicating avoidable subtree teardown/remount and visible flicker.
- Observed behavior in thread route: marking a thread unread from the thread view triggers broad rerender behavior instead of a narrowly scoped state update, indicating mutation/invalidation churn in the thread interaction path.
...
- Browser traces are now captured for all required Phase 1 scenarios.
...
The repository does not currently include a usable `.env` file or mailbox fixture data, but required Phase 1 baseline captures are now recorded: mail-list/thread-open/thread image-heavy/composer browser traces plus mail-list/thread/composer `window.__MAIL_APP_PERF__` traces.